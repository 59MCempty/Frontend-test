      ...
      <div className="flex gap-2">
        {listStatuses.map((s, id) => (
          <button
            key={id}
            id={String(id)}
            onClick={(e) => clickStatusTodo(e, s)}
            className={`rounded-full border border-gray-200 px-6 py-3 text-sm font-bold capitalize transition delay-200 duration-300 ease-out hover:bg-gray-600 hover:text-white ${
              currentId === id
                ...
                <label
                  className={`block pl-3 text-base font-medium ${
                    todo.status === 'completed'
                      ? 'cursor-pointer capitalize text-gray-500 line-through'
                      : ''
                  }`}
